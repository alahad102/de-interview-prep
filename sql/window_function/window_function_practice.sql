...
SELECT
    ...
    avg(revenue) over (
        PARTITION BY
            region
        ORDER BY
            revenue_month ROWS BETWEEN 2 PRECEDING
            AND CURRENT ROW
    ) as moving_avg
FROM
    monthly_revenue as m;
...
window
    w as (
        partition by
            region
        order by
            revenue_month rows BETWEEN unbounded PRECEDING
...
SELECT
    ...
    salary,
    hire_date,
    (YEAR (hire_date) - first_employee_year) as joining_after
FROM
    (
        SELECT
            *,
            FIRST_VALUE (year (hire_date)) OVER (
                PARTITION BY
                    department_id
                ORDER BY
                    hire_date
            ) as first_employee_year
        FROM
            employees
        ORDER BY
            department_id,
